...
[1-9]+(\.{DIGIT}+)[[e|E]([\+\-]?[1-9]{DIGIT}+)     { create_and_store_token(FLOAT_NUMBER,  yytext, line_num); 
...
[a-z]+[A-Z]?[a-zAZ0-9]*(_[a-zAZ0-9]+)* { create_and_store_token(OTHER_ID,  yytext, line_num); 
                       fprintf(yyout,"Line %d : found token of type ID , lexeme %s\n", line_num, yytext);  
                       return 1;
 	      }
.       			fprintf(yyout ,"Unrecognized character!: %s in line %d\n", yytext, line_num);

<<EOF>>       			{ create_and_store_token(EOF_TOK,"", line_num);
						fprintf(yyout ,"Line %d : End of file ", yytext, line_num);
						return 0;}
